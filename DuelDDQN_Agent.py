...
class Agent:
    ...
    def choose_action(self, observation):
        if np.random.random() > self.epsilon:
            state = torch.tensor(observation, dtype=torch.float).unsqueeze(0).cuda()
    ...
    def learn(self):
        ...
        state, action, reward, next_state, done = [x[0] for x in batch], [x[1] for x in batch], [x[2] for x in batch], [
                x[3] for x in batch], [x[4] for x in batch]
        states = torch.tensor(state, dtype=torch.float32).cuda()
        rewards = torch.tensor(reward, dtype=torch.float32).unsqueeze(1).cuda()
        dones = torch.tensor(done, dtype=torch.long).unsqueeze(1).cuda()
        actions = torch.tensor(action, dtype=torch.long).unsqueeze(1).cuda()
        next_states = torch.tensor(next_state, dtype=torch.float32).cuda()

        V_s, A_s = self.q_eval(states)
        V_s_, A_s_ = self.q_next(next_states)
        V_s_eval, A_s_eval = self.q_eval(next_states)
        ...
        q_next[dones] = 0.0
        q_target = rewards + self.gamma * q_next.gather(1, max_actions.unsqueeze(1))
